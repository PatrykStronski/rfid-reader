...
try:
    ...
    cur.execute("CREATE TABLE IF NOT EXISTS card_logs(id SERIAL PRIMARY KEY,uid VARCHAR(100), log_time BIGINT, reader_id VARCHAR(100));")
    conn.commit()
except:
...
def write_message(uid,log_time,reader):
    cur = conn.cursor()
    cur.execute("INSERT INTO card_logs(uid,log_time,reader_id) VALUES('"+str(uid)+"',"+str(log_time)+",'"+str(reader)+"');")
    conn.commit()
